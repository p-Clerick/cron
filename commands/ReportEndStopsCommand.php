    ...
    public function run($dateToRecalc)
    {
        ...
        foreach ($arrPc as $rout => $value) {
            ...
            foreach ($value as $n => $value1) {
                ...
                foreach ($value1 as $key => $value2) {
                    if ($n && ($key == 'direct') && ($arrPc[$rout][$n]['direct'] != $arrPc[$rout][$n - 1]['direct']) ) {
        ...
        for ($cd = 0; $cd < $countDate; $cd++) {
            ...
            if ($rewiew[$cd] != null) {        //якщо треба перерахувати вручну за якийсь день
                ...
                ReportEndStops::model()->deleteAll([
                    'condition' => 'date = :d',
            ...
            if ($countA == 0) {
                ...
                foreach ($a as $key => $value) {
                    ...
                    $arraySchedules[$value['schedules_id']] = [
                        $value['routes_id'],
                        $value['graphs_id'],
                    ];
                }
                foreach ($arrayFakt as $key => $value) {
                    $b = ScheduleTimes::model()->findAll([
                    ...
                    foreach ($arrayNePopav as $sced => $arsch) {
                        foreach ($arsch as $fl => $stat) {
                            foreach ($stat as $key => $value) {
                                for ($q = 1; $q <= 20; $q++) {
                                    if (isset($arrkin[ $arraySchedules[$sced][0] ][$q]) && $value == $arrkin[ $arraySchedules[$sced][0] ][$q]) {
