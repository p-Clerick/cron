...
class ReportSpeedCommand extends CConsoleCommand
{
	public function run($dateToRecalc) {
		...
		$rewiew=$dateToRecalc;
 		...
 		$borts=Borts::model()->findAll(array(
			'order'    => 'id'));
		foreach ($borts as $k) {
			$arrayBorts[]=$k->id;
		}
 		...
 		for ($cd=0; $cd < $countDate; $cd++) {
			...
			for ($bi=0; $bi < count($arrayBorts) ; $bi++) { 
				$locations=Locations::model()->findAll(array(
						...
						'params'   =>array(':f'=>$tdfrom[$cd], ':t'=>$tdto[$cd], ':bid'=>$arrayBorts[$bi]),
						'order'    => 'unixtime'));
				foreach ($locations as $k) {
					$arraySpeedForOneBorts[$k->graphs_id][]=array(
						'lat'=>$k->latitude,
						'long'=>$k->longitude,
						'unixtime'=>$k->unixtime,
						'speed'=>$k->speed
				...
				if (isset($arraySpeedForOneBorts)) {
					...
					ReportSpeedBorts::model()->deleteAll(array(
		    			'condition' => 'date = :d AND borts_id = :b',
						'params' => array(':d' => $dayToCalc[$cd], ':b' => $arrayBorts[$bi])));
