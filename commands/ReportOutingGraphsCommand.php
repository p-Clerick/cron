	...
	public function run($dateToRecalc) {
		...
		for ($cd=0; $cd < $countDate; $cd++) {
			...
			for ($i=0; $i < $countAllGraphs; $i++) {
				$e = LocationsFlights::model()->find(array(
					'select'=>'borts_id',
					'distinct'=>true,
					'condition'=> 'graphs_id = :grid AND unixtime >= :f AND unixtime <= :t',
					'params'   =>array(':grid' => $arrayInsert[$i]['graphId'], ':f'=>$todayFrom[$cd], ':t'=>$todayTo[$cd])/*,
					'order'    => 'unixtime DESC'*/));
				if (isset($e)) {
				...
				if (!isset($e)) {
					...
					$planOut=Orders::model()->findAll(array(
							'condition'=> 't.graphs_id = :gi AND t.from = :f AND t.to = :t',
							'params'   =>array(':f'=>$dayToCalc[$cd], ':t'=>$dayToCalc[$cd],':gi'=>$arrayInsert[$i]['graphId'])/*,
							'order'    => 't.id'*/));
			...
			foreach ($statusBortsOut as $bortId => $value) {
				if (($value['statusOut']!='check') && ($value['st']=='yes')) {
					$arrayInsert[]=array(
						'bortId'=>$value['bortId'],
			...
			ReportOutingGraphs::model()->deleteAll(array(
	    			'condition' => 'date = :d',
