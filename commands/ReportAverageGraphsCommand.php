            ...
            if ($countSql==0) {
                ...
                if (isset($arrayDiff)) {
                    foreach ($arrayDiff as $key => $value) {
                        foreach ($value as $key1 => $value1) {
                            ...
                            $diffaver[$key][$key1]=100-$minusaver[$key][$key1]-$plusaver[$key][$key1];
                        }
                    }
                }
                ...
                foreach ($countAll as $key => $value) {
                    foreach ($value as $key1 => $value1) {
                        ...
                        $w->routes_id=$key;
                        $w->graphs_id=$key1;
                        $w->lateness_percentage=$minusaver[$key][$key1];
                        $w->lateness_average=$avMinus[$key][$key1];
                        $w->advance_percentage=$plusaver[$key][$key1];
                        $w->advance_average=$avPlus[$key][$key1];
                        $w->ontime_percentage=$diffaver[$key][$key1];
                        $w->ontime_average=$avDiff[$key][$key1];
                        $w->save();
                    }
                }
                unset($arrayDiffPlus);
