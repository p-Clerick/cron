...
class WorktimeChartController extends BaseChartController
{

	protected $limit;

	protected $graph_id = 'worktime';
	...
	protected function getSeries ($categories){
		...
		if ($_POST['chart_id'] == 'graphic'){
			foreach($series_name as $s_name => $s_value){
				$series[$i]['name'] = 'Борт '.$s_value['bort'];
					if(isset($s_value)){
						$series[$i]['data'][] = $s_value['percent'];
					}else{
						$series[$i]['data'][] = NULL;
				}
			}
			...
			foreach($series_name as $s_name => $s_value){
				...
				for($j=0; $j<=count($categories)-1;++$j){
					if($s_value['bort'].'-Борт' == $categories[$j])
					$series[$i]['data'][$j] = $s_value['percent'];
